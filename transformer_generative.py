...
class MidiTrainingModule(pl.LightningModule):
    ...
    def generate(self, step) -> dict:
        ...
        with torch.no_grad():
            output_seq = torch.as_tensor(self.tokenizer.encode('\n'))
            while (
                    len(output_seq) < self.seq_len - 1 and predicted_token.unsqueeze(-1)[0] != 104
            ):
                ...
                logits = lm_logits[-1, :]
                top_k, top_p, temperature = 0, 0.96, 1
    ...
    def training_step(self, batch, batch_nb):
        ...
        model_out, loss = self.forward(batch['input_ids'], batch['target_ids'], batch['attention_mask'])
    ...
    def validation_step(self, batch, batch_nb):
        ...
        """
        model_out, loss_val = self.forward(batch['input_ids'], batch['target_ids'], batch['attention_mask'])
